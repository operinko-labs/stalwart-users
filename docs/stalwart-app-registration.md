...
Stalwart downloads the zip from the Resource URL, unpacks it into the data directory under `user-management/`, and serves the SPA at `/manage/`. The `<base href="/">` tag in `index.html` is rewritten by Stalwart to match the URL prefix (`/manage/`).

The SPA's `fetchAPI()` detects it's running under `/manage` and sets `API_BASE` to `/manage/api`, so API calls go to `/manage/api/accounts` etc., which the HTTPRoute forwards to the sidecar container on port 3000.
...
After saving, navigate to `https://stalwart-admin.vaderrp.com/manage/` and confirm:
